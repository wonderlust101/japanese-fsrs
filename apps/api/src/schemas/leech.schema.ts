 ...
 *  do not move a row in the index). `mostLapses` adds a non-immutable head key
 *  (`cards.lapses`), so its cursor encodes that lapse count as a tiebreaker. */
export const leechSortEnum = z.enum(['mostRecent', 'oldestUnresolved', 'mostLapses'])
