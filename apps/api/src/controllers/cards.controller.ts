...
export const listCrossDeck: RequestHandler = async (req, res): Promise<void> => {
	...
	if (query.deckId !== undefined)
		params.deckId = query.deckId;
	if (query.jlptLevel !== undefined)
		params.jlptLevel = query.jlptLevel;
	if (query.status !== undefined)
		params.status = query.status;
	if (query.missingField !== undefined)
		params.missingField = query.missingField;
	if (query.presentField !== undefined)
		params.presentField = query.presentField;
	if (query.pitchPattern !== undefined)
		params.pitchPattern = query.pitchPattern;
