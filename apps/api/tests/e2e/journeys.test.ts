...
import { describeIntegration, isIntegrationEnabled, restSeed, restSelect, restDelete, signInUser } from '../integration/_helpers'
...
async function addCard(u: JourneyUser, fieldsData: Record<string, unknown>): Promise<{ id: string; state: number; version: number }> {
  ...
  if (res.status !== 201) throw new Error(`addCard failed: ${res.status} ${JSON.stringify(res.body)}`)
  return res.body
}
...
describeIntegration('E2E — premade copy isolation', () => {
  it('copies a premade deck into fresh personal cards and leaves the source untouched', async () => {
    ...
    await restSeed('premade_decks', {
      id: premadeId, name: `E2E Premade ${Date.now()}`, description: null,
      deck_type: 'vocabulary', jlpt_level: 'N5', domain: null, is_active: true,
    })
    const sourceIds = [randomUUID(), randomUUID()]
    await restSeed('cards', sourceIds.map((id, i) => ({
      id, user_id: null, deck_id: null, premade_deck_id: premadeId,
      layout_type: 'vocabulary', fields_data: { word: `源${i}`, reading: `げん${i}`, meaning: `source ${i}` },
      jlpt_level: 'N5',
    })))
    ...
    try {
      ...
      const sources = await restSelect<{ id: string; user_id: string | null; deck_id: string | null }>(
        'cards', `premade_deck_id=eq.${premadeId}&user_id=is.null&select=id,user_id,deck_id`,
      )
