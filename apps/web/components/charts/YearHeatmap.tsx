...
function cellLabel(day: HeatmapDay): string {
  ...
  if (day.count === 0) return `${date}: no reviews`
...
export function YearHeatmap({ days }: YearHeatmapProps): React.JSX.Element | null {
  ...
  const model = useMemo(() => {
    ...
    for (let w = 0; w < weeks; w += 1) {
      ...
      if (d.getUTCDate() <= 7) monthMarks.push({ week: w, month: d.getUTCMonth() })
    }
  ...
  for (let i = 0; i < cells.length; i += 1) {
    if (cells[i] != null) { if (firstRealI < 0) firstRealI = i; lastRealI = i }
  }
  ...
  const handleKeyDown = (e: React.KeyboardEvent): void => {
    ...
    e.preventDefault()
    if (target !== cur && isReal(target)) selectAt(target)
    else if (sel === null) selectAt(cur)
  }
    ...
    <figure className="flex flex-col gap-y-3">
      ...
      <div className="relative w-full">
        <svg
          ...
          onFocus={() => { if (sel === null && lastRealI >= 0) selectAt(lastRealI) }}
          onBlur={() => setSel(null)}
            ...
            <div className="mt-0.5 tabular-nums">
              ...
              {sel.day.count > 0 && (
                <span className="ml-2 text-faded-sumi">
                  {Math.round(sel.day.retention * 100)}% retention
                </span>
              )}
            </div>
          </div>
      ...
      <figcaption className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1 font-mono text-sm tabular-nums text-faded-sumi">
        <span>
          Calendar activity <span className="text-sumi-ink/70">·</span> this year
        </span>
