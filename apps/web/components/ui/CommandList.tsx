...
export function CommandList<TValue extends string>({
  items, onSelect, onEscape, inputRef, ariaLabel,
  empty, initialIndex, touchTargets, className,
  ...
  useEffect(() => {
    ...
    if (activeId !== undefined) input.setAttribute('aria-activedescendant', activeId)
    ...
    >
      {items.map((item, idx) => {
        const headerToRender = item.header !== undefined && !renderedHeaders.has(item.header)
          ? item.header
          : null
        if (headerToRender !== null) renderedHeaders.add(headerToRender)
        ...
        return (
          <li key={item.value} className="list-none">
            ...
            <div
              ...
              onMouseEnter={() => {
                if (item.disabled !== true) setActiveIndex(idx)
              }}
              onClick={() => {
