...
import type { ApiBulkCardMutationResult, ApiCard, ApiCardListItem, ApiCrossDeckCardListItem, ApiList, ApiReviewedCard, CardMissingField, CardPresentField, CardSortDir, CardSortField, CardStatusFilter, CreateCardPayload, CrossDeckJlptFilter, GeneratedCardData, GeneratedMnemonic, GeneratedSentences, PitchPattern, UpdateCardPayload } from "@fsrs-japanese/shared-types";
import {
...
export async function listCardsCrossDeckAction(
	...
	if (options.jlptLevel !== undefined && options.jlptLevel !== "all")
		params.set("jlptLevel", options.jlptLevel);
	if (options.status !== undefined && options.status !== "all")
		params.set("status", options.status);
	if (options.missingField !== undefined)
		params.set("missingField", options.missingField);
	if (options.presentField !== undefined)
		params.set("presentField", options.presentField);
	if (options.pitchPattern !== undefined)
		params.set("pitchPattern", options.pitchPattern);
