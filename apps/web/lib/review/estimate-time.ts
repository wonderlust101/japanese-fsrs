...
export function estimateSessionSeconds(cards: ReadonlyArray<ApiDueCard>): number {
  ...
  for (const c of cards) {
    if      (c.state === FSRS_NEW)        secs += SECS_NEW
    else if (c.state === FSRS_LEARNING)   secs += SECS_LEARNING
    else if (c.state === FSRS_RELEARNING) secs += SECS_RELEARNING
...
export function formatSessionEstimate(totalSeconds: number): TimeEstimate {
  if (totalSeconds <= 0) return { totalSeconds: 0, label: '0 min' }
  if (totalSeconds < 60) return { totalSeconds, label: 'under a minute' }
