...
export function useReviewQueue(): readonly ApiDueCard[] {
	return useReviewSessionStore(s => (s.phase === "active" ? s.queue : EMPTY_QUEUE));
}

export function useCurrentCard(): ApiDueCard | undefined {
	return useReviewSessionStore(s => (s.phase === "active" ? s.queue[s.currentIndex] : undefined));
}

export function useCurrentIndex(): number {
	return useReviewSessionStore(s => (s.phase === "active" ? s.currentIndex : 0));
}

export function useShowAnswer(): boolean {
	return useReviewSessionStore(s => (s.phase === "active" ? s.showAnswer : false));
}

export function useSessionHistory(): readonly SessionHistoryEntry[] {
...
export function useIsSessionStarted(): boolean {
