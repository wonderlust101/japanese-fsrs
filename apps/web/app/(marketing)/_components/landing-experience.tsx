      ...
      <Face>
        ...
        <div data-gen-line className="mt-4 rounded-xs bg-cream-inset px-4 py-3">
          <p lang="ja" className="font-japanese text-base leading-relaxed text-sumi-ink">
            毎日<FuriganaText text="練習" reading="れんしゅう" />する。
          </p>
    ...
    face: (
      <Face>
        ...
        <p lang="ja" className="mt-5 font-japanese text-3xl leading-relaxed text-sumi-ink">
          <FuriganaText text="毎朝" reading="まいあさ" />コーヒーを<FuriganaText text="飲" reading="の" />みます。
        </p>
    ...
    face: (
      <Face>
        ...
        <p className="mt-4 text-base leading-[1.6] text-sumi-ink">
          ...
          that’s a <span lang="ja" className="font-japanese">友</span>, a friend who walks with you.
        </p>
  ...
  useGSAP(
    () => {
      ...
      mm.add('(min-width: 1024px) and (prefers-reduced-motion: no-preference)', () => {
        ...
        if (curveFill) gsap.set(curveFill, { opacity: 0 })
        if (curveDot) gsap.set(curveDot, { autoAlpha: 0 })
        ...
        const setStep = (idx: number): void => {
          ...
          if (stepCount) stepCount.textContent = `${idx + 1} of ${acts.length}`
        }
        setStep(0)
        ...
        acts.forEach((act, i) => {
          ...
          if (copy) tl.fromTo(copy, { autoAlpha: 0, x: -36 }, { autoAlpha: 1, x: 0, duration: 0.6, ease: EXPO_OUT }, label)
          if (face) {
            tl.fromTo(
              face,
              flip
          ...
          if (i === acts.length - 1 && curvePath) {
            ...
            if (curveFill) tl.to(curveFill, { opacity: 0.08, duration: 0.5 }, label)
            if (curveDot) tl.to(curveDot, { autoAlpha: 1, duration: 0.2 }, '>-0.1')
          }
        })
      })
              ...
              <div className="flex flex-col gap-5">
                ...
                <Card variant="surface">
                  <p lang="ja" className="font-japanese text-lg leading-relaxed text-sumi-ink">
                    朝ごはんを<FuriganaText text="食" reading="た" />べる。
                  </p>
