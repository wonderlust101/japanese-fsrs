...
import { formatExactCount } from './dashboard-format'
        ...
        <div className={`min-w-0 w-full ${contentClassName}`}>
          ...
          {action !== undefined && (
            <Link
              ...
              className="dashboard-motion-colors group mt-4 inline-flex min-h-11 max-w-full flex-wrap items-center gap-2 break-words font-mono text-xs tracking-wide text-inari-vermillion underline-offset-4 hover:text-inari-vermillion-deep hover:underline focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-inari-vermillion/45"
            >
              <span>{action.label}</span>
              <span
                aria-hidden="true"
                className="dashboard-motion-transform group-hover:translate-x-0.5"
              >
                →
              </span>
            </Link>
  ...
  return (
    <motion.div
      ...
      className={[
        ...
        'dashboard-motion-colors hover:border-error/35 hover:bg-error-tint/35',
      ].join(' ')}
    >
      <span className="dashboard-motion-colors absolute inset-x-4 top-4 h-px bg-error/25 group-hover/error:bg-error/35" />
      <span className="dashboard-motion-colors absolute inset-x-6 bottom-4 h-px bg-error/20 group-hover/error:bg-error/30" />
      <span className="dashboard-motion-preview absolute left-4 top-5 flex h-5 w-5 items-center justify-center rounded-full border border-error/35 bg-error-tint font-mono text-[0.625rem] font-semibold text-error-deep group-hover/error:-translate-y-0.5 group-hover/error:border-error/50">
        !
      </span>
      <span className="dashboard-motion-preview absolute right-4 top-5 font-mono text-[0.625rem] uppercase tracking-[0.12em] text-error-deep/65 group-hover/error:text-error-deep">
        {label}
      </span>
      {children}
    </motion.div>
    ...
    >
      ...
      >
        <div className="min-w-0">
          ...
          <Link
            href={retryHref}
            className="dashboard-motion-colors group mt-4 inline-flex min-h-11 max-w-full flex-wrap items-center gap-2 break-words font-mono text-xs tracking-wide text-error-deep underline-offset-4 hover:text-error hover:underline focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-error/45"
          >
