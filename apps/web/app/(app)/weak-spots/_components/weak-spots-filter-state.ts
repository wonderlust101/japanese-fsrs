...
export function parseWeakSpotFiltersFromURL(
	...
	if (jlpt !== null && JLPT_TOKENS.has(jlpt))
		next.jlptLevel = jlpt;
...
export function serializeWeakSpotFiltersToURL(state: WeakSpotFilters): URLSearchParams {
	...
	if (state.jlptLevel !== "all")
		out.set("jlpt", state.jlptLevel);
	if (state.diagnosis !== "all")
		out.set("diag", state.diagnosis);
	if (state.sort !== "mostRecent")
