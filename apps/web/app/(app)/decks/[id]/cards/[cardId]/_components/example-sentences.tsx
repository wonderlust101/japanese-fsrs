  ...
  return (
    <section className="bg-[var(--color-surface-raised)] rounded-[var(--radius-lg)] shadow-[var(--shadow-card)] p-5 space-y-4">
      <header className="flex items-center justify-between">
        <h2 className="text-xs font-semibold text-faded-sumi uppercase tracking-wider">Example Sentences</h2>
        <Button
          variant="ghost"
          size="sm"
          onClick={regenerate}
      ...
      <div>
        {pending !== null && (
          <p className="text-xs text-faded-sumi mb-2">Current sentences:</p>
        )}
    ...
    <ul className={['space-y-4', dimmed ? 'opacity-60' : ''].join(' ')}>
      {sentences.map((s, i) => (
        <li key={i} className="space-y-0.5">
          ...
          {s.furigana.length > 0 && (
            <p lang="ja" className="text-sm text-faded-sumi leading-[1.9]">{s.furigana}</p>
          )}
          <p className="text-sm text-faded-sumi">{s.en}</p>
        </li>
      ))}
    </ul>
  )
