...
import type { ApiCardListItem, ApiDeck } from "@fsrs-japanese/shared-types";
		...
		<Dialog open={open} onClose={onCancel} title={copy.title} eyebrow={copy.eyebrow} size="xl">
                      ...
                      metadata line. */}
									<div className="min-w-0 flex-1">
										...
										<p className="mt-0.5 truncate font-mono text-sm text-faded-sumi">
											Updated
											{" "}
											<Time value={deck.updatedAt} className="text-sumi-ink/70">{updated}</Time>
										</p>
									</div>
			...
			<div className="mt-5 flex justify-end gap-2">
				...
				<Button
					...
					disabled={!canSubmit}
					onClick={() => {
						if (card !== null && selectedDeckId !== null)
							onConfirm(card, selectedDeckId);
					}}
				>
					{copy.submit}
				</Button>
...
function formatRelativeUpdate(iso: string): string {
	...
	if (diffMs < day * 365)
		return `${Math.floor(diffMs / week / 4)}mo ago`;
