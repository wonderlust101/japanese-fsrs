...
// surface renders it.
interface KanjiEntry { kanji: string; radical: string; meaning: string; reading: string }
...
function fieldsFromCard(card: ApiCard): CardFields {
  ...
  return {
    ...
    sentences: (vf?.exampleSentences ?? []).map((s) => ({ ja: s.ja, en: s.en, furigana: s.furigana })),
    kanjiBreakdown: (vf?.kanjiBreakdown ?? []).map((k) => ({ kanji: k.kanji, radical: k.radical ?? '', meaning: k.meaning, reading: k.reading })),
...
export function EditCardClient({ card, deckName, decks }: EditCardClientProps): React.JSX.Element {
  ...
  }, [regeneratingIdx, existingJa, card.id])
...
function KanjiEditor({ entries, onChange }: KanjiEditorProps): React.JSX.Element {
  ...
  const add    = (): void => onChange([...entries, { kanji: '', radical: '', meaning: '', reading: '' }])
