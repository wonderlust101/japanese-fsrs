...
import type { CardMissingField, CardPresentField, CardStatusFilter, PitchPattern } from "@fsrs-japanese/shared-types";
import type { CardsFilterState, ChipDimension, FilterChip, JlptFilter } from "./cards-filter-state";
import type { CommandItem } from "@/components/ui/CommandList";
...
function applyOption(state: CardsFilterState, dim: ChipDimension, value: string): CardsFilterState {
	switch (dim) {
		...
		case "present": {
			...
			if (nextPresent !== "pitch")
				next.pitchPattern = null;
			return next;
		}
		case "pitchPattern":
