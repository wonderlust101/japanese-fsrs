...
const MISSING_TOKENS: ReadonlySet<CardMissingField> = new Set([
  'reading', 'meaning', 'example', 'mnemonic', 'picture', 'nuance', 'pitch',
])
...
export function parseFiltersFromURL(
  ...
  if (view !== null && view.length > 0) next.viewId = view
...
export function serializeFiltersToURL(state: CardsFilterState): URLSearchParams {
  ...
  if (state.jlpt !== 'all')               out.set('jlpt',    state.jlpt)
  if (state.status !== 'all')             out.set('status',  state.status)
  if (state.sort !== 'recent')            out.set('sort',    state.sort)
  ...
  if (state.viewId !== null)              out.set('view',    state.viewId)
  if (state.missingField !== null)        out.set('missing', state.missingField)
  if (state.presentField !== null)        out.set('present', state.presentField)
...
export function mergeViewRecipe(
  ...
  if (recipe.search       !== undefined && base.search.length === 0)   next.search       = recipe.search
  if (recipe.jlptLevel    !== undefined && base.jlpt === 'all')        next.jlpt         = recipe.jlptLevel
  if (recipe.status       !== undefined && base.status === 'all')      next.status       = recipe.status
  if (recipe.sort         !== undefined && base.sort === 'recent')     next.sort         = recipe.sort
  if (recipe.missingField !== undefined && base.missingField === null) next.missingField = recipe.missingField
  if (recipe.presentField !== undefined && base.presentField === null) next.presentField = recipe.presentField
  if (
...
export function stateMatchesRecipe(
  ...
  if (recipe.search       !== undefined && recipe.search       !== state.search)       return false
  if (recipe.jlptLevel    !== undefined && recipe.jlptLevel    !== state.jlpt)         return false
  if (recipe.status       !== undefined && recipe.status       !== state.status)       return false
  if (recipe.sort         !== undefined && recipe.sort         !== state.sort)         return false
  if (recipe.missingField !== undefined && recipe.missingField !== state.missingField) return false
  if (recipe.presentField !== undefined && recipe.presentField !== state.presentField) return false
  if (recipe.pitchPattern !== undefined && recipe.pitchPattern !== state.pitchPattern) return false
  return true
}
