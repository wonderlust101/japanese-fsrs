...
function buildMatureSeries(p: MatureParams): MaturePoint[] {
  ...
  for (let i = p.days - 1; i >= 0; i -= 1) {
    ...
    const shaped = Math.pow(t, p.shape)
...
function deriveMilestones(series: ReadonlyArray<MaturePoint>): MatureMilestone[] {
  ...
  for (const threshold of thresholds) {
    ...
    if (hit !== undefined) hits.push({ count: threshold, date: hit.date })
  }
  return hits
}
...
export function buildStrongFixture(): ProgressData {
  const heatmap = buildHeatmap({
    seed: 7, daysActive: 365, meanRetention: 0.89, meanCount: 32, weekendDip: 0.82,
  })
  const mature = buildMatureSeries({
    seed: 13, days: 365, endNew: 142, endLearning: 84, endYoung: 382, endMature: 624, shape: 1.4,
  })
  return {
    state: 'strong',
    summary: {
...
export function buildDecliningFixture(): ProgressData {
  ...
  const retention: RetentionPoint[] = base.retention.map((p, i, arr) => {
    if (p.retention === null) return p
    const fromEnd = arr.length - 1 - i
    if (fromEnd > 21) return p
...
export function buildLimitedFixture(): ProgressData {
  const heatmap = buildHeatmap({
    seed: 31, daysActive: 9, meanRetention: 0.86, meanCount: 14, weekendDip: 0.95,
  })
  const mature = buildMatureSeries({
    seed: 41, days: 9, endNew: 38, endLearning: 12, endYoung: 4, endMature: 0, shape: 1.0,
  })
  return {
    state: 'limited',
    summary: {
