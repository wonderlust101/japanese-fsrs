...
export function ReviewForecastChart({ data, isLoading }: Props): React.JSX.Element {
  ...
  return (
    <section className="p-5 rounded-[var(--radius-lg)] bg-[var(--color-surface-raised)] border border-soft-hairline shadow-[var(--shadow-card)]">
      ...
      {isLoading ? (
        <div className="h-32 bg-cream-inset rounded animate-pulse" />
      ) : (
              ...
              >
                ...
                <div
                  className={[
                    ...
                    d.isToday ? 'bg-inari-vermillion' : 'bg-inari-vermillion',
                  ].join(' ')}
                  style={{ height: `${Math.max(h, 2)}%` }}
                  aria-label={`${d.label}: ${d.count} reviews`}
                />
                <span className="text-[10px] text-faded-sumi truncate w-full text-center">{d.label}</span>
              </li>
            )
          })}
        </ol>
