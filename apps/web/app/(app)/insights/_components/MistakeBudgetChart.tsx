    ...
    <figure className="flex flex-col gap-y-3">
      ...
      >
        ...
        {yTicks.map((tick, idx) => {
          ...
          const label =
            isZero            ? 'expected'
            : tick === devMax  ? `${Math.round(devMax)} fewer`
            : tick === -devMax ? `${Math.round(devMax)} more`
            : null
          return (
            <g key={idx}>
              <line
        ...
        {(() => {
          ...
          const d = days[i]
          if (d === undefined || d.count === 0) return null
          const slipping = d.delta < 0
      ...
      <ChartFigcaption>
        ...
        <span className="flex flex-wrap items-center gap-x-3 gap-y-1">
          <span>
            You missed <span className="font-medium text-inari-vermillion-deep">{totalActual}</span>
            <span className="text-faded-sumi/80">, expected </span>
            <span className="text-sumi-ink/80">~{totalExpected}</span>
          </span>
          {accuracyPct !== null && (
            <span className="text-sumi-ink/85">{accuracyPct}% accuracy</span>
          )}
        </span>
      </ChartFigcaption>
    </figure>
