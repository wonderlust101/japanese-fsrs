...
export function SecuritySection(): React.JSX.Element {
	...
	async function submitChangePassword(e: React.FormEvent): Promise<void> {
		...
		if (currentPwd.length === 0)
			fieldErrors.currentPwd = "Enter your current password to continue.";
		if (newPwd.length < 8)
			fieldErrors.newPwd = "New password must be at least 8 characters.";
		if (confirmPwd.length === 0)
			fieldErrors.confirmPwd = "Confirm your new password.";
		else if (newPwd !== confirmPwd)
			fieldErrors.confirmPwd = "The new password and confirmation do not match.";
					...
					<div className="flex flex-col gap-y-3 pl-3">
						<Input
							...
							onChange={(e) => {
								...
								if (pwdErrors.currentPwd !== undefined)
									setPwdErrors(p => ({ ...p, currentPwd: undefined }));
							}}
						...
						<Input
							...
							onChange={(e) => {
								...
								if (pwdErrors.newPwd !== undefined)
									setPwdErrors(p => ({ ...p, newPwd: undefined }));
							}}
						...
						<Input
							...
							onChange={(e) => {
								...
								if (pwdErrors.confirmPwd !== undefined)
