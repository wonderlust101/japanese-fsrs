...
function readDockChrome(): DockChromePersisted {
  if (typeof window === 'undefined') return DOCK_CHROME_DEFAULT
  try {
    const raw = window.localStorage.getItem(DOCK_CHROME_KEY)
    if (raw === null) return DOCK_CHROME_DEFAULT
    ...
    return {
      ...
      sections:  (parsed.sections !== undefined && parsed.sections !== null) ? parsed.sections : {},
    }
  } catch {
...
export function DevDockContextProvider({
  ...
  const register = useCallback(({ registration }: RegisterOptions): void => {
    setPanels((prev) => {
      ...
      if (idx === -1) return [...prev, registration]
      const next = [...prev]
  ...
  const setSectionOpen = useCallback((panelId: string, open: boolean): void => {
    setSections((prev) => {
      ...
      if (current !== undefined && current.collapsed === nextCollapsed) return prev
