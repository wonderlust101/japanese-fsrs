...
Recognition, production, reading, and audio cards have separate `generatorParameters` instances with different `request_retention` targets. Do not consolidate them into a single parameter set — each card type has measurably different forgetting curves in Japanese.
