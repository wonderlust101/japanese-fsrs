-- Adds an explicit guard to process_review() that rejects premade source cards
-- (those with user_id IS NULL) before any FSRS state is written.
-- The service layer already checks this; this is the DB-layer defense-in-depth guard.
...
AS $$
BEGIN
  -- Guard: premade source cards have user_id = NULL and must never have FSRS state written.
  -- The WHERE clause below also prevents this implicitly, but being explicit here makes the
  -- intent visible to any future caller that bypasses the service layer.
