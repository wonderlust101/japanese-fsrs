-- =============================================================
-- Migration: 20260528000000_update_rpcs_return_row.sql
--
-- Performance: cut PATCH endpoints from 2 round-trips to 1.
--
-- Both PATCH services currently run an RPC, then a follow-up SELECT to
-- refresh the row for the response (apps/api/src/services/card.service.ts
-- :467-507 and apps/api/src/services/deck.service.ts:191-225). This
-- migration changes the two update RPCs to RETURN the updated row so the
-- service layer can skip the follow-up read.
--
-- Two changes:
--   A. update_card_with_sibling_sync: RETURNS VOID → RETURNS TABLE(...)
--      with a projection mirroring CARD_COLUMNS in card.service.ts
--      (excludes embedding/tokens/parsed_at/embedding_updated_at —
--      those would balloon every PATCH payload with 1536-float arrays).
--   B. update_deck_with_version_check: RETURNS VOID → RETURNS TABLE(...)
--      with a projection mirroring DECK_COLUMNS in deck.service.ts.
--
-- DROP+CREATE rather than CREATE OR REPLACE: PostgreSQL forbids changing
-- a function's RETURNS signature in place. The explicit GRANT EXECUTE
-- TO service_role is re-issued under the new signatures (Supabase's
-- auto-grant doesn't fire for `supabase db push`).
--
-- Body semantics are unchanged from migration 20260525000000 (cards) and
-- 20260526000000 (decks) — same locking, same RAISE EXCEPTION codes,
-- same UPDATE clauses. The only addition is `RETURN QUERY SELECT ...`
-- at the end of each function returning the just-updated row.
-- =============================================================


-- ─── A. update_card_with_sibling_sync — return updated row ────────────────────
...
BEGIN
  ...
  -- Return the freshly-updated target row. Excludes embedding/tokens/parsed_at/
  -- embedding_updated_at to keep the wire payload small (the embedding column
  -- alone is 1536 floats).
  RETURN QUERY
...
-- ─── B. update_deck_with_version_check — return updated row ──────────────────
