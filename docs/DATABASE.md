...
| `archived_at` | `TIMESTAMPTZ` | YES | `NULL` | Non-null = the deck is archived. Carries the moment of archive so the UI can show "archived 3 days ago" without a separate column. Migration `20260622000000_deck_archive.sql`. Set by `POST /api/v1/decks/:id/archive` (idempotent — re-archiving preserves the original timestamp); cleared by `POST /api/v1/decks/:id/unarchive`. Archived decks are excluded from the default `GET /api/v1/decks` listing (`?view=active`), from `get_due_cards`, from `get_review_forecast`, and from drill-candidate selection in `create_weak_spot_drill_session`. Every other write path is gated at the service layer via `assertDeckActive(deckId, userId)` (and the card-keyed `assertCardDeckActive`), which throw 422 `DECK_ARCHIVED`. |
...
- `decks_user_id_active_idx`: `(user_id) WHERE archived_at IS NULL` — narrows the read cost of the default `view='active'` listing (the hot path) without enlarging the unconditional `(user_id, updated_at DESC)` index. Migration `20260622000000_deck_archive.sql`.
...
| `cards_has_picture_idx` | `((fields_data ->> 'picture')) WHERE user_id IS NOT NULL AND (fields_data ->> 'picture') IS NOT NULL` | Supports the `picture` arm of `list_cards_cross_deck` / `count_cards_cross_deck` presence + missing-field filters. Migration `20260624000000_cards_browser_presence_filters.sql`. |
| `cards_has_pitch_idx` | `((fields_data ->> 'pitchPosition')) WHERE user_id IS NOT NULL AND (fields_data ->> 'pitchPosition') IS NOT NULL` | Supports the `pitch` arm + the pitch-pattern predicate of the cross-deck filters. Migration `20260624000000`. |
...
**Partial unique index:** `weak_spots_card_user_unresolved_idx`: `UNIQUE (card_id, user_id) WHERE resolved = FALSE` — prevents duplicate unresolved weak spot records under race conditions. Note: PG treats `NULL` as distinct in UNIQUE, so multiple orphan rows (where `card_id = NULL` after card deletion) can coexist for the same `(user_id, resolved=FALSE)` combo, which is correct (they don't logically conflict). Renamed from `leeches_card_user_unresolved_idx` by migration `20260615000000_rename_leeches_to_weak_spots.sql`.

**Other indexes:**
- `weak_spots_card_id_idx`: `(card_id)` — per-card lookup. (Renamed from `leeches_card_id_idx` by migration `20260615000000`.)
- `weak_spots_user_id_unresolved_idx`: `(user_id) WHERE resolved = FALSE` — "list user's open weak spots" hot path. The unique partial index can't serve this because it leads with `card_id`. (Renamed from `leeches_user_id_unresolved_idx` by migration `20260615000000`.)
- `weak_spots_session_id_idx`: `(session_id) WHERE session_id IS NOT NULL` — session-summary join. (Renamed from `leeches_session_id_idx` by migration `20260615000000`.)

**Application-side guard:** Weak spot detection runs inside `process_review` (and `process_review_batch`) via the `IF p_lapses >= p_weak_spot_threshold` block. Do **not** add weak spot checks elsewhere or you'll get duplicates.
...
| `id` | `UUID` | NO | `gen_random_uuid()` | Primary key. |
| `user_id` | `UUID` | NO | — | FK to `profiles(id)` — cascades on user deletion. |
| `source` | `TEXT` | NO | — | What populated the queue. CHECK admits `unresolved_weak_spots`, `high_lapse_candidates`, `deck_scoped`, `manual_selection`, `current_card`; Stage 3 only writes the first two. Wire payloads use camelCase (`unresolvedWeakSpots`, `deckScoped`); the service layer maps. |
| `source_query` | `JSONB` | NO | `'{}'::jsonb` | Frozen filter snapshot (`deckId`, `jlptLevel`, `order`, `limit`) for analytics. CHECK enforces `jsonb_typeof = 'object'`. |
...
| `weak_spot_id` | `UUID` | YES | — | FK to `weak_spots(id)` — `SET NULL` on weak spot deletion. Renamed from `leech_id` by migration `20260709000000_rename_leech_to_weak_spot_physical.sql`. |
...
| `source_reason` | `TEXT` | NO | — | Why this card was queued. CHECK admits `unresolved_weak_spot`, `high_lapse_candidate`, `manual_selection`, `current_card`; Stage 3 only writes `unresolved_weak_spot`. |
...
- `weak_spot_drill_session_cards_weak_spot_idx`: `(weak_spot_id) WHERE weak_spot_id IS NOT NULL` — per-weak spot drill history. Renamed from `weak_spot_drill_session_cards_leech_idx` (which was itself renamed from `leech_drill_session_cards_leech_idx`) by migration `20260615000000_rename_leeches_to_weak_spots.sql`.
...
| `weak_spot_id` | `UUID` | YES | — | FK to `weak_spots(id)` — `SET NULL` on weak spot deletion. Always sourced from the session-card row server-side, never from the request body. Renamed from `leech_id` by migration `20260709000000_rename_leech_to_weak_spot_physical.sql`. |
| `card_id` | `UUID` | YES | — | FK to `cards(id)` — `SET NULL` on card deletion. Same server-side sourcing as `weak_spot_id`. |
...
- `weak_spot_drill_attempts_weak_spot_created_idx`: `(weak_spot_id, created_at DESC) WHERE weak_spot_id IS NOT NULL` — per-weak spot drill history. Renamed from `leech_drill_attempts_leech_created_idx` by migration `20260615000000_rename_leeches_to_weak_spots.sql`.
...
| `process_review_batch(p_user_id, p_reviews JSONB, p_weak_spot_threshold)` | Batches N `process_review` calls into one round-trip. Per-review subtransactions preserve "collect errors, continue" semantics. Migration `20260620000000_process_review_batch_return_log_id.sql` added `review_log_id` to the return shape (captured via `RETURNING id INTO v_log_id`) so callers can propagate the per-row log id without a second round-trip. | Returns `(card_id, success, error_message, due, stability, difficulty, scheduled_days, state, review_log_id)` per row. |
...
| `copy_user_deck(p_user_id, p_source_deck_id, p_target_name)` | Atomic duplicate of a user-owned deck. Migration `20260621000000_copy_user_deck_rpc.sql`. Inserts a new `decks` row (default name `<source> (Copy [N])` when `p_target_name IS NULL`, resolved via a unique-name loop; `source_premade_id` is NOT carried — a copy of a premade-derived deck is not itself premade-derived for attribution purposes), then bulk-clones every non-suspended source card with fresh FSRS state. Tags, `is_suspended`, and `parent_card_id` are dropped on the clone; embeddings carry over so similarity search works day-one. Premade source decks (`user_id IS NULL`) and cross-user attempts both fail closed as `deck_not_found` (no ownership leak). | Returns `(deck_id, card_count)`. Raises `deck_not_found` (→ 404) on missing/foreign decks. |
...
| `get_review_forecast(p_user_id, p_days DEFAULT 14, p_timezone DEFAULT 'UTC', p_daily_review_limit DEFAULT NULL, p_daily_new_cards_limit DEFAULT NULL)` | Learner-local forecast for the next N days. Today's bucket is capped to match `get_due_cards()`: backlog fills first against `remaining_total = daily_review_limit - reviewed_today`, scheduled-today reviews fill the remainder, then new cards fill min(`remaining_new`, what's left of `remaining_total`, inventory). Future days carry actual scheduled `review_count` and a depleting projection of new cards bounded by `daily_new_cards_limit` and remaining inventory. NULL daily limits skip capping (admin/debug only). Migration `20260605000000_review_forecast_daily_caps.sql` reversed the no-projection behaviour introduced by `20260530000000_review_forecast_actual_new_counts.sql`, replacing it with the inventory-bounded projection. Migration `20260623000000_archive_aware_forecast_and_drill.sql` added an `archived_decks` CTE and `c.deck_id NOT IN (SELECT id FROM archived_decks)` to the overdue, new-inventory, and scheduled-per-day CTEs so archived-deck cards no longer contribute to the projection (mirrors `get_due_cards`'s exclusion shape; the two RPCs now stay arithmetically consistent). | `(date TEXT, count BIGINT, backlog_count BIGINT, review_count BIGINT, new_count BIGINT)` |
...
| `create_weak_spot_drill_session(p_user_id, p_source, p_deck_id, p_jlpt_level, p_order, p_limit, p_mode, p_repeat_policy, p_stop_rule, p_source_query, p_card_ids, p_card_id, p_min_lapses)` | Stage 3 of the weak spot-drill feature; Stage 4 replaced the body to call the fingerprint helper instead of an inline expression; **Stage 6 added three new parameters** (`p_card_ids` for `manual_selection`, `p_card_id` for `current_card`, `p_min_lapses` for `high_lapse_candidates`) and expanded the candidate-selection CTE to a four-branch `UNION ALL` so all five spec source values are now wired through. Migration `20260623000000_archive_aware_forecast_and_drill.sql` added an `archived_decks` CTE and `c.deck_id NOT IN (SELECT id FROM archived_decks)` to all four branches so cards in archived decks never enter a fresh drill. The `deck_scoped` / `current_card` branches are also hard-gated at the controller layer (`assertDeckActive` / `assertCardDeckActive`) — the RPC-level filter is defense-in-depth there; for the library-spanning `unresolved_weak_spots` / `high_lapse_candidates` / `manual_selection` branches the silent skip turns a session that would otherwise 422 on every card into a smaller-but-actionable drill. The Stage 4 fingerprint helper is unchanged — same byte-for-byte output, existing sessions' stored hashes stay valid. Inserts one `weak_spot_drill_sessions` row and N `weak_spot_drill_session_cards` rows (snapshots) atomically. Writes nothing to `cards` or `review_logs` — the scheduler-invariance guarantee is structural. | `JSONB` envelope: `{ sessionId, status, cards: [{ sessionCardId, weakSpotId, cardId, ordinal, layoutType, fieldsData, lapses }] }`. |
| `get_weak_spot_drill_session(p_user_id, p_session_id)` | Stage 4 of the weak spot-drill feature. Returns the persisted queue plus an advisory staleness signal computed by recomputing each card's fingerprint via the helper and comparing against the stored baseline on `weak_spot_drill_session_cards`. Orphan rows (card deleted post-snapshot, `card_id IS NULL`) are surfaced via `cardId: null` + `isOrphaned: true` and are NOT counted as stale — there is nothing to compare to. RAISEs `weak_spot_drill_session_not_found` with SQLSTATE `02000` when the session is missing or owned by another user; service layer translates to HTTP 404 `WEAK_SPOT_DRILL_SESSION_NOT_FOUND`. Reads `cards` and `weak_spot_drill_session_cards` only — no writes to either, no FSRS touched. | `JSONB` envelope: `{ sessionId, status, isCanonicalStateStale, staleCards: [cardId...], cards: [{ sessionCardId, weakSpotId, cardId, ordinal, layoutType, fieldsData, lapses, isOrphaned, isStale }] }`. |
| `record_weak_spot_drill_attempt(p_user_id, p_session_id, p_event_id, p_session_card_id, p_asserted_card_id, p_asserted_weak_spot_id, p_result, p_local_sequence, p_response_time_ms, p_shown_at, p_answered_at)` | Stage 5 of the weak spot-drill feature. (1) Reads canonical `card_id` / `weak_spot_id` from `weak_spot_drill_session_cards` keyed on `(session_card_id, session_id)`; verifies the user owns the session-card. RAISEs `weak_spot_drill_session_card_not_found` (SQLSTATE `02000`) on triple mismatch → service maps to HTTP 404 `WEAK_SPOT_DRILL_SESSION_CARD_NOT_FOUND`. (2) Compares the optional body-side `p_asserted_card_id` / `p_asserted_weak_spot_id` against the canonical values; mismatches RAISE `weak_spot_drill_attempt_card_mismatch` or `weak_spot_drill_attempt_weak_spot_mismatch` (SQLSTATE `22000`) → service maps to HTTP 422 `WEAK_SPOT_DRILL_ATTEMPT_ASSERTION_MISMATCH`. (3) INSERTs with `ON CONFLICT (user_id, event_id) DO NOTHING` for idempotent replay; the row's `weak_spot_id`/`card_id` are always the canonical values, never the body's. (4) Returns the canonical attempt envelope. Reads `weak_spot_drill_session_cards` and `weak_spot_drill_attempts` only — no writes to `cards`, `review_logs`, or any other canonical FSRS table. | `JSONB` envelope: `{ attemptId, eventId, sessionId, sessionCardId, weakSpotId, cardId, result, localSequence, responseTimeMs, shownAt, answeredAt, createdAt }`. |
| `transition_weak_spot_drill_session(p_user_id, p_session_id, p_target_status)` | Stage 6 of the weak spot-drill feature. Flips `weak_spot_drill_sessions.status` from `'active'` to the requested terminal state (`'finished'` or `'aborted'`) atomically. `FOR UPDATE` lock on the session row guards against concurrent transitions. Idempotent on no-op retries: re-finishing a finished session (or re-aborting an aborted one) returns successfully without touching `finished_at` (preserves the first-finish timestamp). Rejects illegal transitions with `weak_spot_drill_session_state_conflict` (SQLSTATE `22000`) → service maps to HTTP 409 `WEAK_SPOT_DRILL_SESSION_STATE_CONFLICT`. Missing session → `weak_spot_drill_session_not_found` (SQLSTATE `02000`) → HTTP 404. Writes only `status`, `finished_at`, `updated_at` on `weak_spot_drill_sessions` — no reads or writes against `cards` or `review_logs`. | `VOID`. The service calls `get_weak_spot_drill_session` afterwards to return the post-state envelope, so the wire shape matches the Stage 4 GET response. |
