...
- [x] **Sentence-layout AI generator branch (Backend Completion Plan Stage 13, 2026-05-17)** — `POST /api/v1/decks/:deckId/cards` with `mode='ai'` + `layoutType='sentence'` now dispatches to a new sentence-card generator that produces the Stage 12 shape. Decided on a **separate `generateSentenceCard` function** (over a discriminator inside `generateCard`) because the prompts diverge enough — vocabulary asks for `word`/`reading`/`meaning` + the back-of-card composition; sentence asks for `ja`/`en`/`furigana` + `breakdown` + `nuance`. Two clean functions with separate Redis cache namespaces (`card:v2:…` vs `sentence-card:v1:…`) keeps each prompt focused and lets the version bumps move independently. New `SENTENCE_CARD_PROMPT_VERSION = 'v1'` constant baked into the sentence-card cache key (same pattern as `CARD_PROMPT_VERSION` and `DIAGNOSIS_PROMPT_VERSION`). New `GeneratedSentenceCardSchema` in `packages/shared-types/src/schemas/ai.schema.ts` mirrors `SentenceFieldsDataSchema` — required `ja`/`en`/`furigana` plus optional `breakdown`/`nuance`/`audio` (the `audio` field is admitted but unmapped at the prompt layer for the same asset-hosting reason `picture`/`expressionAudio` stay out of the vocabulary prompt). Controller `apps/api/src/controllers/cards.controller.ts` dispatches on `input.layoutType === 'sentence'` inside the `mode==='ai'` branch. `createCard` service signature widened to accept `FieldsData | GeneratedCardData | GeneratedSentenceCard`. Re-exports for `GeneratedSentenceCardSchema` + `GeneratedSentenceCard` added in `packages/shared-types/src/index.ts`. **No DB migration** — Stage 12 already shipped the schema admittance + DB CHECK; this stage only adds an AI generator branch that feeds into the same Stage 12-validated create path. Pre-Stage-13 a user request `{ mode: 'ai', layoutType: 'sentence' }` was effectively broken: the system generated a vocabulary card shape and tried to insert it as `layout_type='sentence'`, which now fails the Stage 12 CHECK (23514). Coverage: 6 unit tests in `apps/api/src/services/__tests__/ai.service.test.ts` (cached payload round-trips at the `sentence-card:v1:…` key shape, separate cache namespace verified, three missing-key rejections on `GeneratedSentenceCardSchema`, full payload with breakdown + nuance round-trip) plus 2 end-to-end integration tests in `apps/api/tests/integration/cards.routes.test.ts` that pre-seed the live Redis cache with a valid sentence-card payload at the canonical key, POST with `mode='ai' layoutType='sentence'`, and assert the canonical shape survives the Stage 12 DB CHECK (the cache-seeded approach proves the controller dispatch + service cache-key composition + DB CHECK roundtrip without burning OpenAI tokens). The second integration test pins the cross-namespace isolation: vocabulary requests don't read from sentence-card cache and vice versa. `afterAll` cleans up the seeded Redis keys so stale entries don't leak into subsequent runs. The vocabulary path is unchanged (same `generateCard` invocation, same cache key). **Frontend follow-ups:** manual sentence-card creation form (the AI path is the primary surface; manual is available but no dedicated UI) and sentence review render rules.
