...
- [ ] **Reconcile IA card types with `card_type` schema**
  - IA omits Listening and conflates `layout_type` with `card_type`. Decide: ship Listening for MVP or defer it explicitly across IA + canonical docs.
- [ ] **Resolve "Problem Card" vs "Leech" vocabulary**
  - Backend says "leech"; IA renames to "Problem Card Repair". Pick one label before the frontend lands (PRODUCT.md edit required if "Problem Card" wins).
...
- [x] **Problem-card list (lapse-bucketed) (Stage 7, 2026-05-17)** — `GET /api/v1/insights/problem-cards?bucket={2-3|4-5|6-7|8plus}`. Migration `20260608000000_get_problem_cards_rpc.sql`. Acceptance: `8plus` cardinality equals the unresolved-leech count (integration test pins it).
...
- [x] **Rename Leeches → Weak spots in user-facing surfaces (2026-05-17)** — route `/insights/leeches/*` → `/insights/weak-spots/*` (via `git mv`); all 25 URL references rewritten; kanji ornament 蛭 → 弱. Internal code (components, hooks, store, query keys) keeps `leech*` to stay aligned with `/api/v1/leeches`.
- [x] **Retire /insights/mistakes (2026-05-17)** — page deleted after the IA restructure. Card quality bars moved to `/cards` behind a `欠 Card quality` toggle; pattern summary + problem-cards bars removed as redundant; confusables deferred; leeches mini-list canonicalized at `/insights/weak-spots`.
- [x] **Leeches Phase 2 — Drill flow (2026-05-17)** — full drill experience under `/insights/leeches/drill/{setup,[sessionId],[sessionId]/summary}`. Bespoke 3-channel `DrillRatingBar` (Missed/Hesitated/Remembered); `useLeechDrillSessionStore` (Zustand); scheduler-invariance honored. Phase 3 deferred: dashboard surface + "Count this as a review" override.
- [x] **Leeches sidebar entry + unresolved-count badge (2026-05-17)** — `useUnresolvedLeechCount()` projection over `useLeechesQuery`; vermillion-stroked count chip capped at 50. Other nav rows pay zero subscription cost.
- [x] **Leeches Phase 1 — List + Detail + Diagnosis (2026-05-17)** — `/insights/leeches` (later `/insights/weak-spots`) lists unresolved/resolved leeches with six filter dimensions; per-row Drill / View / Resolve actions; detail `Dialog` includes `LeechDiagnosisPanel` calling `POST /api/v1/leeches/:id/diagnose` with fresh `Idempotency-Key`.
...
- [x] **Review Summary redesign (2026-05-15)** — closure-first page composed of `SectionCard`s with a six-pattern state classifier (strong / mixed / difficult / leech / ended-early / no-pattern). Session "End session" routes to Summary for symmetric calm close.
...
- [x] **Leeches list + detail (Stage 1)** — `GET /api/v1/leeches` (filters, sorts, tuple cursor) + `GET /:id`; orphans preserved via LEFT/INNER join switching. Commit `81a0b35` (2026-05-14).
- [x] **Leeches resolve + reopen (Stage 2)** — idempotent flips; partial-unique-index 23505 → HTTP 409 `LEECH_ALREADY_OPEN`.
- [x] **Leeches list spec-alignment patch (Stage 2.5)** — adds `deckOrder` sort + `diagnosis: available | missing` filter.
...
- [x] **Free MVP completion + AI leech diagnosis (Stage 7)** — paid/free tier model removed; `POST /:id/diagnose` ships free; writes only `leeches.diagnosis` + `leeches.prescription`.
