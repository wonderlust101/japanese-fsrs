...
- [x] **Add Japanese capture redesign (2026-05-15)** — implements `docs/information_architecture/06_add_japanese.md`. Replaces the `/add` stub with a single-`SectionCard` capture surface (kanji `採` / label `CAPTURE` / vermillion stripe / date-seeded teacher line via `CAPTURE_LINES`, mirroring `today-hero`'s `PREPARATION_LINES` rhythm). Two primary fields: `Input` with `script="mixed"` (Word) and a new `Textarea` primitive — `apps/web/components/ui/Textarea.tsx`, identical chrome to `Input` (cream-inset / soft-hairline / rounded-[2px] / `script` prop) — rendering the sentence as a block-padded notebook surface. Optional fields (Image, Note, Deck, Source, Card type) live as quiet inline `· `-separated chips via a new `AddChipRow` component with `aria-expanded` / `aria-controls` and focus-on-promote behavior; filled chips carry a vermillion dot indicator. Empty-sentence flow keeps the primary button copy stable ("Create card") and surfaces a passive `role="status"` warning above the action row after blur or first click — second click submits anyway. Image chip is wired end-to-end in the UI (data-URL preview, 5MB cap, MIME-restricted picker, remove affordance) but deliberately omitted from the downstream request body for this iteration. `Cmd/Ctrl+Enter` submits from anywhere; the draft is persisted to a new `useCaptureDraftStore` (Zustand, devtools-wrapped) and the page routes to `/add/review`, which owns the eventual create-card API call (Add stays a pure capture surface per the IA). Verified via typecheck + lint + build (`/add` ships at 7.08 kB).
