...
- [ ] **Leeches frontend wiring (Phase 2 — drill flow)**
	  - Phase 1 (List + Detail + Diagnosis) shipped 2026-05-17 — see Done. Remaining: build the `LeechDrillSetup` → `LeechDrillSession` → `LeechDrillSummary` flow against the existing `POST /api/v1/leeches/drill-sessions` endpoint family (already shipped, Stages 1–8 backend). Also: dashboard leeches card and `hasLeeches` signal on Today (the list query can derive it via `limit=1` + `items.length > 0`).
