...
- [ ] **Onboarding deck recommendations (frontend slice — in progress)**
  - **Shipped 2026-05-18:** `/onboarding/decks` no longer holds a hardcoded `RECOMMENDED_DECKS` array. It now calls `usePremadeDecks` and filters the live catalogue against the JLPT level chosen on the earlier `/onboarding/level` step (`beginner` and `null` both fall through to N5; N1 also pulls in `beyond_jlpt`). Level-agnostic decks (jlpt_level NULL — e.g. Joyo Kanji) sit beneath the matching JLPT row in the recommendations list. Loading skeletons + empty state for "no decks match this level" replace the static-array UX. **Remaining:** layer interest-based scoring on top, and route the user's selections through `POST /api/v1/premade-decks/:id/copy` on continue (right now the page still just persists profile prefs and routes to `/today`).
...
- [x] **/cards browser — full live wire-up (2026-05-18)** — `/cards` no longer runs on dev fixtures. New backend endpoints land: `GET /api/v1/cards/cross-deck` (search + deck/jlpt/status/missing-field filters + recent|due|lapses sort + cursor pagination), `POST /:id/move`, `POST /:id/copy`, `POST /:id/suspend`, `POST /:id/unsuspend`, plus bulk endpoints (`bulk/move`, `bulk/suspend`, `bulk/unsuspend`, `bulk/delete`, `bulk/tag`). All idempotency-keyed; bulk endpoints return per-id `{succeeded, failed}` so the UI can show partial success. Migrations `20260617000000_list_cards_cross_deck_rpc.sql` and `20260617000001_card_move_copy_suspend_rpcs.sql` carry nine new SECURITY DEFINER RPCs. Frontend: `apps/web/lib/api/cards.ts` exposes the new query + mutation hooks, `cards-browser-view.tsx` was rewritten to drive off live data, row actions (edit / delete / move / add-copy) now do the real thing, `CardsBulkBar` is mounted on selection, `CardsQualityBars` reads `GET /api/v1/insights/card-quality`, the "more filters" toggle still surfaces a calm not-yet-shipped notice for pitch/has-image dims. Saved views ship as a small set of curated presets (`recent`, `low-mastery`, `suspended`, `missing-mnemonic`/`example`/`picture`) persisted by id in `localStorage`. Quality bars deep-link with `?missing=<field>` on initial mount.
