...
| Tomo daily note insight/idiom API | Missing | The current dashboard uses a temporary `PracticeSignal` unavailable state. Product intent is to restore this area to Tomo daily notes later; no `/api/v1/tomo/note` route, web action, API hook, paid insight source, or curated idiom source was found. |
