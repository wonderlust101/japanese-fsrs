...
Refreshed by code inspection on 2026-05-14 (Stage 8 + docs cleanup: removed duplicate "AI leech diagnosis" Partial row already covered by the Stage 7 Implemented row, and the "Dashboard weekly summary API" Missing row — superseded by heatmap-derived seven-day activity, no surface needs the endpoint). This records current implementation evidence, not product intent. Product truth lives in [PRODUCT.md](PRODUCT.md), design truth in [DESIGN.md](DESIGN.md), database truth in [DATABASE.md](DATABASE.md), and active tasks in [KANBAN_BOARD.md](KANBAN_BOARD.md).
