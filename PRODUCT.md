...
- The **free tier** includes the FSRS scheduler, premade decks (JLPT N5 through N1, Beyond JLPT, Joyo Kanji, grammar patterns), manual card creation, all chrome, all analytics, full offline review, and full keyboard and screen-reader accessibility. A learner who pays nothing has a complete, working SRS for Japanese.
- The **paid tier** unlocks the AI features: automatic card generation from a single word or sentence, personalized contextual example sentences tuned to the learner's interests and grammar level, AI-written mnemonics, and leech diagnosis with prescriptive next-step guidance.

The free tier is not crippled. It is the same Tomo, with manually-prepared cards. The paid tier is the upgrade for learners who want the AI working alongside them, not a punishment for learners who don't.
...
This is deliberate, and it has a precise commercial consequence. The AI features are the paid tier (see Business Model), but the upgrade is **quality and depth**, not visible AI labeling. Most freemium AI products fall into the trap of needing to brand "AI" everywhere to justify the price; Tomo refuses that trap. We trust learners to value sharper cards, sentences that fit their interests, and mnemonics that stick. We do not need a sparkle icon to prove the AI is doing its job.
...
1. **Two registers, one identity.** The product register is primary today; the brand register (a future v1.1+ landing surface) carries equal strategic weight when it ships. Both share the same voice, type system, and emotional temperature. The brand surface is not the product surface with bigger fonts, it has its own job (identity statement), but a stranger should recognize the same Tomo if they cross from one to the other. The kitsune mark and Muted Berry Red are the literal connective tissue between the two registers.
...
7. **Invisible AI, visible craft.** The intelligence is everywhere in the cards but nowhere in the chrome. The learner pays for AI-tier features and receives sharper cards, more personal sentences, and stickier mnemonics, not more visible "AI" branding. If a future surface is tempted to add a sparkle icon, an "AI mode" toggle, or a "Generated by GPT" footer to justify the paid tier, that surface is wrong. The justification is in what the AI *makes*, not what it is *labeled.*

## Accessibility & Inclusion

- **WCAG 2.1 AA** is the floor (per PRD §2.4). Strive for AAA on text contrast in long-form reading surfaces (sentences, mnemonics, grammar explanations).
- **Reduced motion** is honored end-to-end. The `prefers-reduced-motion` media query already disables animations in `globals.css`; that contract holds for all future motion (opt-out only, never opt-in animation).
...
- **Full keyboard navigation** for review sessions (per PRD §2.4). Number keys 1–4 rate, space/enter reveals, escape pauses. The mouse is optional; the keyboard is canonical.
...
- **Free-tier completeness.** Accessibility is not a paid feature. Every accessibility commitment in this section applies equally to the free tier and the paid tier. Reduced motion, keyboard navigation, screen-reader pronunciation, and color-blind safety must never be gated.
